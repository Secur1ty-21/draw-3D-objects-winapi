...
void CEditBox::OnText(TCHAR* char_, size_t lenght) {
	int i = 0;
	if (char_ != L"")
	{
		while (char_[i])
		{
			m_sText.insert(m_sText.begin() + i, char_[i]);
			i++;
		}
	}
	else 
...
CEditBox* CForm::GetEditBox2(int id_)
{
...
bool SetTextFormEditBox()
{
	if (count < 5)
	{
		if (CEditBox* peb = g_form.GetEditBox2(count))
		{
			CEditBox* peb0 = g_form.GetEditBox2(5);
			int lenght = peb0->m_sText.size();
			wchar_t* wString;
			if (!(peb0->Text().empty()))
			{
				wStrings[count] = (wchar_t*)malloc(sizeof(wchar_t*) * lenght);
				wString = (wchar_t*)peb0->Text().c_str();
				for (int i = 0; i < lenght; i++)
					wStrings[count][i] = wString[i];
				wStrings[count][lenght] = L'\0';
				peb->OnText(wStrings[count], sizeof(TCHAR*) * lenght);
				peb0->OnText((wchar_t*)L"", lenght);
				count++;
			}
			return true;
		}
...
bool RemoveTextFormEditBox()
{
	CEditBox* peb = g_form.GetEditBox2(0);
	CEditBox* peb0 = g_form.GetEditBox2(1);
		if (!(peb->Text().empty()) && count >= 0 && count < 10)
		{
			for (int i = 0; i < 10; i++)
			{
					...
					if (wStrings[i + 1] != 0 && i + 1 != 10)
					{
						...
						if (i < 5)
						{
							peb0 = g_form.GetEditBox2(i);
							peb = g_form.GetEditBox2(i + 1);
							int lenght = peb0->m_sText.size();
							int n_lenght = peb->m_sText.size();
							if (!(peb0->Text().empty()))
							{
								peb0->OnText(wStrings[i], lenght);
								peb0->OnText(wStrings[i + 1], sizeof(TCHAR*) * n_lenght);
							}
						}
					...
					else
					{
						...
						if (i < 5)
						{
							CEditBox* peb0 = g_form.GetEditBox2(i);
							if (!(peb0->Text().empty()))
							{
								int lenght = peb0->m_sText.size();
								peb0->OnText(wStrings[i], sizeof(TCHAR*) * lenght);
							}
						}
					}
			}
