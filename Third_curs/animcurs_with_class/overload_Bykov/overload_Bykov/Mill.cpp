...
MillClass mill;
LPCTSTR g_szWindowClassName = _T("Mill_WindowClass");
HDC g_clone;
HWND hwnd_;
...
void DrawTruss(WPARAM wParam_, LPARAM lParam_){
	...
	//
	mill.Full_Movement(mill.g_alpha, mill.g_move_x, mill.g_move_y, mill.g_zoom, &ferm[0], 8, 17);
	//
	...
	//
	mill.checkPerimetr(ferm, 13, height, width);
	//
...
LRESULT CALLBACK WindowProc(HWND hWnd_, UINT nMsg_, WPARAM wParam_, LPARAM lParam_){
	...
	case WM_SIZE:
		...
		return 0L;
	case WM_TIMER:
		mill.g_alpha += 2 * M_PI / 180.0;
		mill.g_move_x += 2 * mill.g_znak_x;
		mill.g_move_y += 2 * mill.g_znak_y;
		mill.g_zoom += 0.01 * mill.g_znak;
		if (mill.g_zoom >= 1.5)
			mill.g_znak = -1;
		if (mill.g_zoom <= 0.5)
			mill.g_znak = 1;
